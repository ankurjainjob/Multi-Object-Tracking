classdef trajectoryGaussianFilteringForm
    ...
    methods (Static)
        ...
        function density_pred = predict(density, motionmodel)
            ...
            %INPUT: state: a structure with two fields:
            %                   x: object state mean --- (state dimension) x trajectory length matrix
            %                   P: object state covariance --- (state dimension x state dimension x trajectory length) matrix
            %       motionmodel: a structure specifies the motion model parameters
            %OUTPUT:state_pred: a structure with two fields:
            %                   x: predicted object state mean --- (state dimension) x trajectory length matrix
            %                   P: predicted object state covariance --- (state dimension x state dimension x trajectory length) matrix
            
            density_pred.x = [density.x motionmodel.f(density.x(:,end))];
            density_pred.P = cat(3,density.P,motionmodel.F(density.x(:,end))*density.P(:,:,end)*motionmodel.F(density.x(:,end))'+motionmodel.Q);
        ...
        function density_upd = update(density_pred, z, measmodel)
            ...
            %                   P: updated object state covariance --- (state dimension x state dimension x trajectory length) matrix
            
            %Measurement model Jacobian
            Hx = measmodel.H(density_pred.x(:,end));
            %Innovation covariance
            S = Hx*density_pred.P(:,:,end)*Hx' + measmodel.R;
            %Make sure matrix S is positive definite
            S = (S+S')/2;
            
            K = (density_pred.P(:,:,end)*Hx')/S;
            
            %State update
            density_upd.x = [density_pred.x(:,1:end-1) density_pred.x(:,end) + K*(z - measmodel.h(density_pred.x(:,end)))];
            %Covariance update
            density_upd.P = cat(3,density_pred.P(:,:,1:end-1),(eye(size(density_pred.x(:,end),1)) - K*Hx)*density_pred.P(:,:,end));
        ...
        function [z_ingate, meas_in_gate] = ellipsoidalGating(density_pred, z, measmodel, gating_size)
            ...
            S = measmodel.H(density_pred.x(:,end))*density_pred.P(:,:,end)*measmodel.H(density_pred.x(:,end))' + measmodel.R;
            %Make sure matrix S is positive definite
            S = (S+S')/2;
