...
for k = 1:K
    [~,I] = sort(GNNestimates{k}(1,:));
    GNNestimates{k} = GNNestimates{k}(:,I);
end
...
GNNestimates_ref = GNNfilter(tracker_ref, initial_state, measdata, sensor_model, motion_model, meas_model);
GNN_RMSE_ref = RMSE_n_objects(objectdata.X,GNNestimates_ref);


for k = 1:K
    [~,I] = sort(GNNestimates_ref{k}(1,:));
    GNNestimates_ref{k} = GNNestimates_ref{k}(:,I);
end

% Compare.
assert(abs(GNN_RMSE-GNN_RMSE_ref) < 1e-3, 'Root Mean Square Error: Expected %f, got %f. Your GNN implementation is incorrect, please modify your code and submit again.', GNN_RMSE_ref, GNN_RMSE);
assessVariableEqual('GNNestimates', GNNestimates_ref,'RelativeTolerance',0.03,'Feedback','Your GNN implementation is incorrect, please modify your code and submit again.');